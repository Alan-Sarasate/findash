...
describe("GETBYID transaction", () => {
    it("Buscando transação existente", async() => {
        ...
        const res3 = await request(app).get(`/transactions/${transactionId}`).send()
        expect(res3.statusCode).toEqual(StatusCodes.OK)
        expect(res3.body).toHaveProperty('id')
    })

    it("Bucando transação inexisttente", async () => {
        const res1 = await request(app).get('/transactions/999999').send()
