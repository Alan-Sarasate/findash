...
describe("UPDATE Category", () => {
    it('Atualizando categoria existente', async () => {
        ...
        const res2 = await request(app).put(`/categories/${res1?.body?.id}`).send({name: "Testando edição", type: "expense"})
        expect(res2.statusCode).toEqual(StatusCodes.OK)
    })

    it("Atualizando registro que não existe", async () => {
        const res3 = await request(app).put('/categories/999999').send({name: "Categoria teste", type: "income"})
